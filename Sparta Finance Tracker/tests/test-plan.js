...
(async () => {
  ...
  section('12. the skin: blue at the top, app background at the bottom');
  const skin = await page.evaluate(() => {
    const el = document.querySelector('.plan-skin');
    const cs = getComputedStyle(el);
    return { img: cs.backgroundImage,
             before: getComputedStyle(el, '::before').backgroundImage,
             anims: document.getAnimations().filter(a => a.animationName === 'planskin').length };
  });
  // getComputedStyle drops "180deg" because top-to-bottom is the default
  // direction, so verticality is asserted from the rendered pixels below, not
  // from the string. What the string can prove is that the old 115deg diagonal
  // is gone and no horizontal direction crept in.
  check(/linear-gradient/.test(skin.img) && !/deg|to (right|left)/.test(skin.img),
    'a plain top-to-bottom gradient, no leftover diagonal', skin.img.slice(0, 46));
  check(skin.img.includes('rgb(30, 95, 224)'), 'starts on the reference blue #1E5FE0');
  ...
  // sample top vs bottom of the panel to prove the fade actually renders
  const px = async fy => {
    const b = await page.evaluate(f => { const r = document.querySelector('.plan-skin').getBoundingClientRect();
      return { x: Math.round(r.x + r.width - 30), y: Math.round(r.y + r.height * f) }; }, fy);
  ...
  const top = await px(0.02), bot = await px(0.97);
  const lum = ([r, g, b]) => 0.2126 * r + 0.7152 * g + 0.0722 * b;
  check(lum(top) > lum(bot) * 3, 'the top really is far brighter than the bottom',
    `top ${top} vs bottom ${bot}`);
  check(bot[2] < 60 && bot[0] < 40, 'the bottom has settled into the dark app background', String(bot));
